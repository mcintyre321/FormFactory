    ...
    public static class FF
    {
        ...
        public static IEnumerable<PropertyVm> GetPropertyVmsUsingReflection(object model, Type fallbackModelType)
        {
            ...
            foreach (var property in properties)
            {
                ...
                if (!(type.GetTypeInfo().GetMethod(property.Name + "_show")?.Invoke(model, null) as bool? ?? true))
                    continue;
                if (!(type.GetTypeInfo().GetProperty(property.Name + "_show")?.GetValue(model) as bool? ?? true))
                    continue;
