...
namespace FormFactory
{
    ...
    public class PropertyVm : IHasDisplayName
    {
            ...
            this(pi.PropertyType, pi.Name)
        {
            ...
            if (model != null)
            {
                MethodInfo choices = model.GetType().GetTypeInfo().GetMethod(pi.Name + "_choices");
                if (choices != null)
                {
                    Choices = (IEnumerable)choices.Invoke(model, null);
                }
                MethodInfo suggestions = model.GetType().GetTypeInfo().GetMethod(pi.Name + "_suggestions");
                if (suggestions != null)
                {
